...
export const GlobalStorage = ({ children }) => {
  ...
  const getProducts = async (category) => {
    const response = await fetch(`https://my-json-server.typicode.com/xxdyoh/latihan-talent/${category}`);
  ...
  React.useEffect(() => {
    async function loadData() {
      const response = await fetch('https://my-json-server.typicode.com/xxdyoh/latihan-talent/makanan');
      const json = await response.json();
      setData(json);
    }
    loadData();
